...
def fix_omnipath_csv_for_nichecompass(file_path: str) -> None:
    """
...
par = {
    ...
    # collectri params
    "input_collectri_tf_network": None,
...
if par["create_omnipath_gene_program_mask"]:
    ...
    lr_network_file_path = par["input_omnipath_lr_network"] or par["output_omnipath_lr_network"]
    ...
    # Fix OmniPath CSV format for compatibility with load_from_disk (workaround for library bug)
    if save_to_disk:
        fix_omnipath_csv_for_nichecompass(lr_network_file_path)
...
if par["create_collectri_tf_gene_program_mask"]:
    ...
    # Use input file path if provided, otherwise use output file path
    tf_network_file_path = par["input_collectri_tf_network"] or par["output_collectri_tf_network"]
