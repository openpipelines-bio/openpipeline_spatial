...
meta = {
  "temp_dir": "resources_test/gene_programs/",
...
if par["create_omnipath_gene_program_mask"] and par["species"] == "mouse" and not par["input_gene_orthologs_mapping_file"]:
    raise ValueError("For mouse species, a --input_gene_orthologs_mapping_file file must be provided for generating the omnipath mask.")
if par["create_nichenet_gene_program_mask"] and par["species"] == "mouse" and not par["input_gene_orthologs_mapping_file"]:
    raise ValueError("For mouse species, a --input_gene_orthologs_mapping_file file must be provided for generating the nichenet mask.")
if par["create_nichenet_gene_program_mask"] and not par["input_ligand_target_matrix_file"]:
    raise ValueError("For nichenet gene program mask, an --input_ligand_target_matrix_file file must be provided.")
if par["create_mebocost_gene_program_mask"] and (not par["input_metabolite_enzymes"] or not par["input_metabolite_sensors"]):
    raise ValueError("For mebocost gene program mask, both --input_metabolite_enzymes and --input_metabolite_sensors files must be provided.")
...
if par["create_omnipath_gene_program_mask"]:
    ...
    plot_gp_gene_count_distributions = True if par["output_omnipath_gp_gene_count_distributions"] else False
    ...
    omnipath_gp_dict = extract_gp_dict_from_omnipath_lr_interactions(
        ...
        gp_gene_count_distributions_save_path=par["output_omnipath_gp_gene_count_distributions"])
...
if par["create_nichenet_gene_program_mask"]:
    ...
    plot_gp_gene_count_distributions = True if par["output_nichenet_gp_gene_count_distributions"] else False
    ...
    nichenet_gp_dict = extract_gp_dict_from_nichenet_lrt_interactions(
        ...
        gene_orthologs_mapping_file_path=par["input_gene_orthologs_mapping_file"],
        plot_gp_gene_count_distributions=plot_gp_gene_count_distributions,
        gp_gene_count_distributions_save_path=par["output_nichenet_gp_gene_count_distributions"])
...
if par["create_mebocost_gene_program_mask"]:
    ...
    plot_gp_gene_count_distributions = True if par["output_mebocost_gp_gene_count_distributions"] else False

    mebocost_gp_dict = extract_gp_dict_from_mebocost_ms_interactions(
        dir_path=meta["temp_dir"],
        species=par["species"],
        plot_gp_gene_count_distributions=plot_gp_gene_count_distributions,
        gp_gene_count_distributions_save_path=par["output_mebocost_gp_gene_count_distributions"]
    )
...
if par["create_collectri_tf_gene_program_mask"]:
    ...
    plot_gp_gene_count_distributions = True if par["output_collectri_tf_gp_gene_count_distributions"] else False
    ...
    collectri_gp_dict = extract_gp_dict_from_collectri_tf_network(
        ...
        save_to_disk=save_to_disk,
        plot_gp_gene_count_distributions=plot_gp_gene_count_distributions,
        gp_gene_count_distributions_save_path=par["output_collectri_tf_gp_gene_count_distributions"]
    )
