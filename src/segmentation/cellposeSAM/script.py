...
par = {
    ...
    "channels": None,
    # Parameters
    "pretrained_model_name": "cpsam_v2",
    ...
    # Outputs
    "output": "foo.zarr",
    "output_labels": "cellposeSAM_labels",
}
...
if image_arr.ndim == 3:
    ...
    if par["channels"]:
        raise ValueError(
            "'--channels' was provided but the input image has no channel axis."
        )
    channel_axis = None

## Load model
if par["pretrained_model_file"]:
    logger.info(f"Loading custom pretrained model '{par['pretrained_model_file']}'...")
    model = models.CellposeModel(
        gpu=par["use_gpu"], pretrained_model=par["pretrained_model_file"]
    )
else:
    logger.info(f"Loading built-in model '{par['pretrained_model_name']}'...")
    model = models.CellposeModel(
        gpu=par["use_gpu"], pretrained_model=par["pretrained_model_name"]
    )

## Run segmentation
logger.info("Running Cellpose-SAM segmentation...")
masks, flows, styles = model.eval(
    ...
    max_size_fraction=par["max_size_fraction"],
    niter=par["niter"],
    augment=par["augment"],
